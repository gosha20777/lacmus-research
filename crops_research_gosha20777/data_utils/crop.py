...
class DatasetGridCropper:
    ...
    def generate_dataset(self, fragments_type) -> None:
        ...
        if fragments_type == 'pedestrian':
            print('Filter pedestrian fragments...')
            fragments_pedestrian = self._filter_fragments_pedestrian(fragments)

            print('Saving pedestrian fragments...')
            ids = self._write_fragments_pedestrian(fragments_pedestrian)
        
        if fragments_type == 'background':
            print('Filter background fragments...')
            fragments_background = self._filter_fragments_background(fragments)

            print('Saving background fragments...')
            ids = self._write_fragments_background(fragments_background)
    ...
    def _filter_fragments_pedestrian(self, fragments: List[GridFragment]) -> List[GridFragment]:
        """Отбираем фрагменты для датасета
    ...
    def _filter_fragments_background(self, fragments: List[GridFragment]) -> List[GridFragment]:
        """Отбираем фрагменты для датасета
        ...
        #with_human = [f for f in fragments if f.big_annotations()]
        without_human = [f for f in fragments if not f.big_annotations() and not f.small_annotations()]
        without_human = random.choices(without_human, k=int(len(without_human)*0.7))
        return without_human
    ...
    def _write_fragments_pedestrian(self, fragments: List[GridFragment]) -> List[ImageIdType]:
        def save_one_fragment(index: int, frag: GridFragment) -> None:
            with tempfile.NamedTemporaryFile(suffix='.jpg') as t:
                ...
                #self.target_dataset.add(image_id=image_id, source_image_path=file_name, annotations=frag.annotations())
    ...
    def _write_fragments_background(self, fragments: List[GridFragment]) -> List[ImageIdType]:
        def save_one_fragment(index: int, frag: GridFragment) -> None:
            with tempfile.NamedTemporaryFile(suffix='.jpg') as t:
...
def run_crop(source_path: click.Path,
    ...
    cropper.generate_dataset('pedestrean')
